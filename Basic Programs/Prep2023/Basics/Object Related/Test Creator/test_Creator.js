...
let List=(subjectName, difficulty)=>{

    if(!difficultEnum[difficulty]){
        throw new Error("accepted difficultEnum as difficulty");
    }
    
if(Object.keys(hm).length === 0 ){
    for(let sub of entries){
        let [subject,question,difficulty,ans]=sub.split("-");
        difficulty = difficulty && difficulty.trim() ? difficulty.trim() : "Easy" ;//assume
    ...
    hm[subject][difficulty].push(question);
    }
}
    return hm[subjectName][difficulty];
};
...
console.log(List("History",'semi-hard'));

const DIFFICULTY = (function () {
    let en = {};
    en[en["EASY"]=0] = "EASY";
    en[en["MEDIUM"]=1] = "MEDIUM";
    en[en["HARD"]=2] = "HARD";
    return en;
})();
